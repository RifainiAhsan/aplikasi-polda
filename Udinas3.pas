...
procedure TForm_dinas3.DelFilesFrom(Directory, Filemask: string;
  DelSubDirs: Boolean);
...
function TForm_dinas3.waktu: string;
var tgl : TDateTime;
begin
tgl:=Now();
...
procedure TForm_dinas3.frxrprt1BeforePrint(Sender: TfrxReportComponent);
...
procedure TForm_dinas3.FormShow(Sender: TObject);
begin
...
procedure TForm_dinas3.btn1Click(Sender: TObject);
var PDFku : TfrxCustomExportFilter;
namapdf,lokasihapus : string;
begin
  if asalscan = 0 then
  begin
    namapdf :=Form_dinas.edt1.Text+'-'+Form_dinas.edt2.Text+'-'+waktu+'arsipdinas.pdf';
    PDFku:=TfrxCustomExportFilter(frxpdfxprt1);
    PDFku.ShowDialog := False;
    PDFku.FileName:= ExtractFilePath(Application.ExeName)+'\ARAD\'+namapdf;
    frxrprt1.PrepareReport(False);
    frxrprt1.Export(PDFku);
    Form_dinas.lbl8.Caption:=namapdf;
  end;
  Form_dinas2.vrtltbl1.Clear;
